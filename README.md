...
算出したい年を指定することもできます。`!流行語大賞 2025`のように年を半角で数字のみ入力してください。指定できる範囲は2015～2039です。  
年を指定しなかった場合、自動的に年が選択されます。
...
このBotでは、分析対象期間を1年間と設定します。この1年間をさらに1か月ごとに区切り、それぞれの期間に送信された全メッセージを結合して、独立した文書 $d$ として扱います。
この文書 $d$ に対し、TF-IDFを用いて各文書に対して計算を行います。

期間を1か月ごとで区切っている理由として、IDFの重みを下げるといった点が挙げられます。  
...
その対策として、1か月に分割することで、IDFの過度な重みを抑制しています。
